  ...
  return (
    <div className="flex flex-col items-center py-12 px-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-orange-500 text-[10px] font-black uppercase tracking-[0.22em]">Launching</span>
        <span className="w-2 h-2 rounded-full bg-orange-500 animate-ping" />
      </div>
      <p className="text-orange-500 text-2xl sm:text-3xl font-black mb-8">25 March 2026</p>
      {rdy && (
        <div className="flex gap-3 sm:gap-5">
          {[{ v: t.days, l: "Days" }, { v: t.hours, l: "Hours" }, { v: t.mins, l: "Mins" }, { v: t.secs, l: "Secs" }].map(({ v, l }) => (
            <div key={l} className="flex flex-col items-center bg-white border-2 border-orange-100 rounded-2xl px-4 py-4 sm:px-8 sm:py-6 min-w-[64px] sm:min-w-[100px] shadow-sm">
              <span className="text-3xl sm:text-5xl font-black text-orange-500 tabular-nums leading-none">{String(v).padStart(2, "0")}</span>
              <span className="text-[10px] sm:text-xs font-black text-orange-300 uppercase tracking-widest mt-2">{l}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="flex flex-col items-center">
      <div className="relative flex justify-center items-center gap-4">
        <button
          ...
          className="hidden sm:flex w-10 h-10 rounded-full bg-white border-2 border-orange-100 items-center justify-center hover:border-orange-400 transition-colors shadow-sm"
        >
          <ChevronRight className="w-5 h-5 text-orange-500 rotate-180" />
        </button>
        <div className="w-[200px] h-[400px] sm:w-[240px] sm:h-[480px] bg-gray-950 rounded-[36px] p-2.5 shadow-2xl border-4 border-white ring-1 ring-orange-100 overflow-hidden">
      ...
      <div className="flex gap-2 mt-5 justify-center">
        {screens.map((_, idx) => (
          <button
            key={idx}
            onClick={() => setI(idx)}
            className={`h-2 rounded-full transition-all ${idx === i ? "w-7 bg-orange-500" : "w-2 bg-orange-200"}`}
          />
        ))}
      </div>
    </div>
...
function UnisMarquee() {
  return (
    <section id="universities" className="py-12 bg-white border-y border-orange-50 overflow-hidden">
      <p className="text-center text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] mb-8">
  ...
  return (
    <div>
      <div className="hidden sm:grid sm:grid-cols-3 gap-5">
        {IMG.reviews.map((r, i) => (
          <div key={i} className="bg-white rounded-2xl border border-orange-100 p-6 flex flex-col items-center text-center shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200">
            <div className="relative mb-4">
              <img src={r.src} alt={r.name} className="w-14 h-14 rounded-full object-cover border-2 border-orange-200" />
      ...
      <div className="sm:hidden">
        <div key={cur} className="bg-white rounded-2xl border border-orange-100 p-6 flex flex-col items-center text-center mx-2 ani-fade shadow-sm">
          <img src={IMG.reviews[cur].src} alt={IMG.reviews[cur].name} className="w-14 h-14 rounded-full object-cover border-2 border-orange-200 mb-3" />
        ...
        <div className="flex justify-center gap-2 mt-4">
          {IMG.reviews.map((_, i) => (
            <button
              key={i}
              onClick={() => setCur(i)}
              className={`h-2 rounded-full transition-all ${i === cur ? "w-5 bg-orange-500" : "w-2 bg-orange-200"}`}
            />
          ))}
        </div>
      </div>
...
function Volunteer() {
  ...
  return (
    <section id="volunteer" className="py-20 bg-white border-t border-orange-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
        <Reveal>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {perks.map(({ Icon, text }, i) => (
              <div key={i} className="flex gap-3 items-center p-3 bg-orange-50 rounded-xl border border-orange-100">
                <span className="flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-xl bg-white border border-orange-200">
        ...
        <Reveal>
          <div className="bg-orange-500 rounded-3xl overflow-hidden shadow-xl shadow-orange-200/60">
            ...
            <div className="grid grid-cols-3 divide-x divide-white/20 bg-orange-600/40">
            ...
            <div className="px-8 py-8 bg-white flex flex-col items-center gap-3">
              <a
                ...
                className="group w-full inline-flex items-center justify-center gap-3 bg-orange-500 hover:bg-orange-600 text-white font-black text-base px-8 py-4 rounded-2xl transition-colors shadow-md shadow-orange-200/60"
              >
...
function About() {
  const pillars = [
    { Icon: Target,    title: "Mission",  text: "Empower every African student with AI-powered commerce — list in 2 minutes, buy at the lowest campus prices.",        bg: "bg-orange-50",  c: "bg-orange-500"  },
    { Icon: Eye,       title: "Vision",   text: "Africa's first AI e-commerce network — 500+ campuses and 1M+ students by 2030, starting in Ghana.",                 bg: "bg-orange-50",  c: "bg-orange-400"  },
    { Icon: Lightbulb, title: "AI-First", text: "RIRI.ai auto-generates listings from photos, detects fakes, and suggests the best price — in under 2 minutes.",     bg: "bg-orange-50",  c: "bg-orange-500"  },
    { Icon: Globe2,    title: "Impact",   text: "From Sunyani to Accra, Kumasi to Tamale — keeping money circulating within African student communities.",            bg: "bg-orange-50",  c: "bg-orange-400"  },
  ]
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center mb-16">
          <Reveal>
            <div className="relative h-[380px] sm:h-[420px]">
              ...
              <div className="absolute bottom-20 left-3 z-10 bg-white rounded-2xl shadow-lg px-4 py-3 border border-orange-100 flex items-center gap-3">
              ...
              <div className="absolute top-3 right-3 z-10 bg-orange-500 text-white rounded-2xl shadow-lg px-4 py-3 text-center">
          ...
          <Reveal>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={i} className="bg-orange-50 rounded-2xl p-4 border border-orange-100 flex items-center gap-3">
                  <span className="flex items-center justify-center w-9 h-9 rounded-xl bg-orange-500 flex-shrink-0"><Icon className="w-4 h-4 text-white" /></span>
        ...
        <Reveal>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {pillars.map(({ Icon, title, text, c, bg }, i) => (
              <div key={i} className={`${bg} rounded-2xl p-6 border border-orange-100 group hover:-translate-y-1 transition-all duration-200`}>
...
// ─── AFRICA'S FIRST — redesigned: orange + white only, human-friendly ────────
function AfricaFirst() {
  return (
    <section id="africa-first" className="py-20 bg-orange-500">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <Reveal className="text-center mb-14">
          <span className="inline-flex items-center gap-2 text-[10px] font-black text-orange-500 bg-white px-5 py-2 rounded-full uppercase tracking-widest mb-6">
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-[1.06] mb-5">
            ...
            <span className="bg-white text-orange-500 px-3 rounded-xl inline-block mt-1">AI-verified</span>{" "}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={title} className="bg-white rounded-2xl p-6 flex gap-4">
                <div className="flex-shrink-0 w-11 h-11 rounded-xl bg-orange-50 border border-orange-200 flex items-center justify-center text-orange-500 mt-0.5">
                  {Icon}
                </div>
                <div>
        ...
        <Reveal>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
            {[
              ...
              <div key={l} className="bg-white rounded-2xl px-5 py-6 text-center">
                <p className="text-3xl sm:text-4xl font-black text-orange-500 leading-none mb-2">{v}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              ...
              className="group inline-flex items-center justify-center gap-2.5 bg-white text-orange-500 font-black px-8 py-4 rounded-2xl hover:bg-orange-50 transition-colors shadow-sm"
            >
              Join the Movement
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </button>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <Reveal className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-gray-900 leading-tight mb-3">
            Everything You <span className="text-orange-500">Need to Know</span>
          </h2>
        ...
        <div className="space-y-2.5">
          {FAQ_ITEMS.map((item, i) => (
            <Reveal key={i}>
              <div className={`rounded-2xl border transition-all duration-200 overflow-hidden ${open === i ? "border-orange-300 bg-orange-50" : "border-gray-100 bg-white hover:border-orange-100"}`}>
                <button
                  onClick={() => setOpen(open === i ? null : i)}
                  className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left"
                >
                  <span className={`text-sm sm:text-base font-black leading-snug ${open === i ? "text-orange-600" : "text-gray-800"}`}>
                    {item.q}
                  </span>
                  <span className={`flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-200 ${open === i ? "bg-orange-500 text-white rotate-180" : "bg-orange-50 text-orange-400"}`}>
...
function Community() {
  return (
    <section id="community" className="py-20 bg-orange-50 border-t border-orange-100">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
        <Reveal>
          <span className="inline-flex items-center gap-2 text-[10px] font-black text-orange-500 bg-white border border-orange-200 px-5 py-2 rounded-full uppercase tracking-widest mb-6">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-orange-100 p-7 sm:p-10">
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              {["Deals", "Textbooks", "Earn Cash", "Launch News"].map(tag => (
                <span key={tag} className="text-xs bg-orange-50 text-orange-600 border border-orange-200 px-3 py-1 rounded-full font-bold">{tag}</span>
              ))}
            </div>
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
...
function UniBot() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-4 sm:right-6 z-50 w-[min(320px,calc(100vw-32px))] ani-fade">
          <div className="bg-white rounded-2xl shadow-2xl border border-orange-100 overflow-hidden">
            {/* Header */}
            <div className="bg-orange-500 px-4 py-3.5 flex items-center gap-3">
            ...
            <div className="p-4">
              ...
              <div className="flex flex-col gap-2 mt-4">
                <a href={LISTING_URL} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-black text-sm px-4 py-3 rounded-xl transition-colors">
                  <Store className="w-4 h-4" /> List a Product Free
                </a>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 bg-white border border-orange-200 text-orange-500 hover:bg-orange-50 font-black text-sm px-4 py-3 rounded-xl transition-colors">
                  <WaIcon className="w-4 h-4 fill-orange-500" /> Join Our Community
                </a>
              </div>
            </div>
          </div>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-5 right-4 sm:right-6 z-50 w-14 h-14 rounded-full bg-orange-500 shadow-xl shadow-orange-300/50 flex items-center justify-center hover:bg-orange-600 transition-colors border-2 border-white focus:outline-none"
...
function HeroMosaic() {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute -bottom-4 -left-3 z-10 bg-white rounded-2xl shadow-lg px-4 py-2.5 border border-orange-100 flex items-center gap-2.5">
      ...
      <div className="absolute -top-3 -right-3 z-10 bg-orange-500 text-white rounded-2xl shadow-lg px-3 py-2.5 text-center">
  ...
  return (
    <>
      ...
      <div className="bg-white text-gray-900 min-h-screen overflow-x-hidden">
        ...
        {/* ── HEADER ── */}
        <header className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-xl border-b border-orange-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-5">
              {links.map(({ l, id }) => (
                <button key={id} onClick={() => go(id)}
                  className="text-sm font-semibold text-gray-500 hover:text-orange-500 transition-colors relative group">
                  {l}
            ...
            <div className="flex items-center gap-2">
              <a href={LISTING_URL} target="_blank" rel="noopener noreferrer"
                className="hidden sm:inline-flex items-center gap-1.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-black px-5 py-2.5 rounded-xl transition-colors shadow-sm shadow-orange-200/60">
                <Store className="w-4 h-4" /> Become a Vendor
              </a>
              <button onClick={() => setNavOpen(v => !v)} className="lg:hidden p-2 rounded-xl hover:bg-orange-50 transition-colors">
          ...
          {navOpen && (
            <div className="lg:hidden bg-white border-b border-orange-50 px-4 py-4 grid grid-cols-2 gap-1 shadow-lg ani-fade">
              {links.map(({ l, id }) => (
                <button key={id} onClick={() => go(id)}
                  className="text-sm font-semibold text-gray-600 hover:text-orange-500 hover:bg-orange-50 px-4 py-3 rounded-xl text-left transition-colors">
                  {l}
                </button>
              ))}
              <a href={LISTING_URL} target="_blank" rel="noopener noreferrer"
        ...
        <section id="hero" className="pt-28 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
            <div className="space-y-6 ani-fade">
              <div className="inline-flex items-center overflow-hidden rounded-full border border-orange-200 bg-white shadow-sm">
                <span className="bg-orange-500 text-white text-[9px] font-black px-3 py-1.5 uppercase tracking-[0.16em] flex-shrink-0 flex items-center gap-1">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.06] text-gray-900">
                ...
                <span className="relative inline-block">
                  ...
                  <svg className="absolute -bottom-1 left-0 w-full" viewBox="0 0 200 8" fill="none" preserveAspectRatio="none">
                    <path d="M4 5 Q50 1 100 5 Q150 9 196 4" stroke="#F97316" strokeWidth="2.5" strokeLinecap="round" fill="none" />
                  </svg>
                </span>{" "}
                campus marketplace
              </h1>

              <div className="bg-orange-500 rounded-2xl p-4 sm:p-5">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={() => go("join-marketplace")}
                  className="group inline-flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-black px-8 py-4 rounded-2xl shadow-md shadow-orange-200/60 transition-colors">
                  Join UniMart Free
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
                <button onClick={() => go("about")}
                  className="inline-flex items-center justify-center gap-2 border-2 border-orange-200 hover:border-orange-400 text-gray-700 hover:text-orange-500 font-black px-8 py-4 rounded-2xl transition-colors">
                  Our Story
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span key={t} className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-600 bg-white px-3 py-1.5 rounded-full border border-orange-100 shadow-sm">
        ...
        {/* ── FEATURES ── */}
        <section id="features" className="py-16 px-4 sm:px-6 lg:px-8 bg-orange-50 border-y border-orange-100">
          <div className="max-w-7xl mx-auto">
            <Reveal className="text-center mb-10">
              <h2 className="text-3xl sm:text-4xl font-black text-gray-900 mb-3">
                Everything traditional e-commerce can't do — <span className="text-orange-500">we do with AI</span>
              </h2>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <Reveal key={title}>
                  <div className="bg-white rounded-2xl p-6 border border-orange-100 flex flex-col items-center text-center h-full shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-200">
        ...
        <section id="how-it-works" className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
          <div className="max-w-5xl mx-auto">
            <Reveal className="text-center mb-12">
              ...
              <h2 className="text-3xl sm:text-5xl font-black text-gray-900 mb-3">AI does the work.<br /><span className="text-orange-500">You just show up.</span></h2>
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                ...
                <Reveal key={n}>
                  <div className="relative bg-orange-50 rounded-2xl p-7 border border-orange-100 text-center hover:-translate-y-1 hover:shadow-md transition-all duration-200">
                    <span className="absolute top-4 right-5 text-5xl font-black text-orange-200 select-none">{n}</span>
                    <div className="w-13 h-13 rounded-2xl bg-orange-500 flex items-center justify-center mx-auto mb-5 w-14 h-14 shadow-md shadow-orange-200/60">
        ...
        {/* ── REVIEWS ── */}
        <section id="reviews" className="py-16 px-4 sm:px-6 lg:px-8 bg-orange-50 border-y border-orange-100">
          <div className="max-w-7xl mx-auto">
            <Reveal className="text-center mb-10">
        ...
        <section id="onboard" className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
          <div className="max-w-5xl mx-auto">
            <div className="bg-orange-500 rounded-3xl p-8 sm:p-12 text-white overflow-hidden relative">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none" />
              <div className="relative z-10 grid md:grid-cols-2 gap-10 items-center">
                <div>
                  <span className="inline-block text-[9px] font-black bg-white text-orange-500 px-3 py-1.5 rounded-full uppercase tracking-widest mb-5">Free for First 500 Vendors</span>
                  ...
                  <a href={LISTING_URL} target="_blank" rel="noopener noreferrer"
                    className="group inline-flex items-center gap-2 bg-white text-orange-500 font-black px-7 py-4 rounded-2xl hover:bg-orange-50 transition-colors shadow-sm">
        ...
        {/* ── WAITLIST ── */}
        <section id="join-marketplace" className="py-20 px-4 sm:px-6 lg:px-8 bg-orange-50 border-t border-orange-100">
            ...
            {popup && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
                <div className="bg-white rounded-3xl shadow-2xl p-10 max-w-sm w-full flex flex-col items-center border border-orange-100 ani-fade">
                  ...
                  <Button onClick={() => setPopup(false)} className="bg-orange-500 hover:bg-orange-600 text-white font-black px-8 rounded-xl">Got it</Button>
                </div>
              </div>
            )}
          </div>
        ...
        <footer className="bg-gray-950 py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto flex flex-col items-center gap-5 text-center">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="block text-xl font-black text-orange-500 leading-none">Uni-Mart</span>
